...
ggplot(res, aes(x = date, y = cor_estimate)) + 
  geom_line(alpha = 0.4) +
  geom_point(data = subset(res, p_value <= 0.05), size = 2) + # "significant"
...
much meaningless and changes between days. But, if we JUST select the 
significant points, we can p-hack! :D 

```{r Plotting Part 2}
# filtering for significant + negative correlations (for phacking interpretation, hehe)
...
Attending boot camps contributes to a sense of community that makes people more 
likely to follow health guidelines to prevent their neighbors from getting sick! 
Community engagement also helps to spread scientific understanding of COVID 
outbreaks and cases.
